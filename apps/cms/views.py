...
from apps.cms.forms import EditNewsCategory, WriteNewsForm, AddBannerForm, EditBannerForm, EditNewsForm
...
class NewsListView(View):
    ...
    def get(self, request):
        ...
        if category_id:
            newses = newses = newses.filter(category=category_id)

        paginator = Paginator(newses, 2)  # 分页对象
...
class EditNewsView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            category = NewsCategory.objects.get(pk=category_id)
            News.objects.filter(pk=pk).create(title=title, desc=desc, thumbnail=thumbnail,
                                              content=content, category_id=category_id, category=category)
            return restful.ok()
        else:
            return restful.params_error(message=form.get_errors())
